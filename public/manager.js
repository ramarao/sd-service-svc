...
let state = { phone: "", name: "", providers: [], providerId: null, providerName: "", tier: null, tab: "orders" };
...
async function tabOrders() {
  ...
  const groups = [
    { key: "REQUESTED", label: "🆕 New requests", match: (s) => s === "REQUESTED" },
    { key: "ACTIVE", label: "In progress", match: (s) => ["ACCEPTED", "ASSIGNED", "PICKED_UP", "IN_SERVICE", "OUT_FOR_DELIVERY"].includes(s) },
    { key: "DONE", label: "Completed", match: (s) => ["DELIVERED", "REJECTED"].includes(s) },
  ];
...
async function orderDetail(id) {
  ...
  if (order.status === "REQUESTED") {
    ...
    if (next === "ASSIGNED" || next === "OUT_FOR_DELIVERY") {
      const isDel = next === "OUT_FOR_DELIVERY";
      const cur = isDel ? order.delivery_captain_name : order.agent_name;
      if (captains.length) {
        const opts = `<option value="">Select a captain…</option>` + captains.map((c) => `<option value="${c.id}" data-name="${esc(c.name || "")}" data-phone="${esc(c.phone || "")}" ${cur === c.name ? "selected" : ""}>${esc(c.name || "Captain")}${c.phone ? " · " + esc(c.phone) : ""}</option>`).join("");
        agentField = `<label>Assign ${isDel ? "delivery" : "pickup"} captain</label><select id="agent">${opts}</select>`;
      } else {
        agentField = `<label>Assign ${isDel ? "delivery" : "pickup"} captain</label><p class="muted small">No captains yet — add them in the Captains tab.</p><input id="agent" value="${esc(cur || "")}" placeholder="Captain's name" />`;
      }
    }
    ...
    controls = `<p class="muted">${order.status === "REJECTED" ? "Rejected — no further action." : "Delivered — complete."}</p>`;
  }
  ...
  h(`
    ...
    <div class="card">
      ...
      ${order.agent_name ? `<p class="muted small" style="margin:6px 0 0">🧢 Pickup: ${esc(order.agent_name)}${order.captain_phone ? ` · <a href="tel:${esc(order.captain_phone)}">${esc(order.captain_phone)}</a>` : ""}</p>` : ""}
      ${order.delivery_captain_name ? `<p class="muted small" style="margin:2px 0 0">🛵 Delivery: ${esc(order.delivery_captain_name)}${order.delivery_captain_phone ? ` · <a href="tel:${esc(order.delivery_captain_phone)}">${esc(order.delivery_captain_phone)}</a>` : ""}</p>` : ""}
    </div>
...
async function tabPayment() {
  ...
  content().innerHTML = `
    ...
    <p class="muted small" style="margin:0 0 12px">Set your UPI ID so captains can show the customer a payment QR after delivery.</p>
