...
function renderLanding() {
  h(`
    <div class="topbar"><h1>Scandeer Service</h1></div>
    <div class="card">
      <p class="muted">On-site service platform — pickup, service, deliver.</p>
      <h2>Are you a…</h2>
...
async function customerNewOrder(slug, provider) {
  ...
  function renderAddrSection(mode) {
    ...
    if (mode === "add" || !addresses.length) {
      box.innerHTML = `
        ...
        <input id="cname" placeholder="Who should we contact at pickup?" />
  ...
  document.getElementById("submit").onclick = async () => {
    const items = [...picker.querySelectorAll(".pick-row")]
      ...
      .filter((i) => i.qty > 0);
    const msg = document.getElementById("msg");
    if (!selectedAddrId) { msg.className = "err"; msg.textContent = "Select or add a pickup address."; return; }
...
async function customerTrack(id) {
  ...
  h(`
    ...
    <div class="card">${badge(order.status)} ${order.delivery_captain_name ? `<span class="muted">· 🛵 Delivery: ${esc(order.delivery_captain_name)}</span>` : order.agent_name ? `<span class="muted">· 🧢 Pickup: ${esc(order.agent_name)}</span>` : ""}
...
async function ordersTab(container) {
  ...
  // KPI tiles: total + one bucket per stage group. Clicking filters the list.
  const TILES = [
    { key: null, label: "Total", accent: true, match: () => true },
    { key: "REQUESTED", label: "New", match: (s) => s === "REQUESTED" },
    { key: "ACTIVE", label: "In progress", match: (s) => ["ACCEPTED", "ASSIGNED", "PICKED_UP", "IN_SERVICE", "OUT_FOR_DELIVERY"].includes(s) },
...
async function adminOrder(id) {
  ...
  if (order.status === "REQUESTED") {
    ...
    let agentField = "";
    // ASSIGNED → pick a pickup captain; OUT_FOR_DELIVERY → pick a delivery captain.
    if (next === "ASSIGNED" || next === "OUT_FOR_DELIVERY") {
      const isDelivery = next === "OUT_FOR_DELIVERY";
      const label = isDelivery ? "Assign delivery captain" : "Assign pickup captain";
      const current = isDelivery ? order.delivery_captain_name : order.agent_name;
      if (captains.length) {
        const opts = `<option value="">Select a captain…</option>` +
          captains.map((c) => `<option value="${c.id}" data-name="${esc(c.name || "")}" data-phone="${esc(c.phone || "")}" ${current === c.name ? "selected" : ""}>${esc(c.name || "Captain")}${c.phone ? " · " + esc(c.phone) : ""}</option>`).join("");
        agentField = `<label>${label}</label><select id="agent">${opts}</select>`;
      } else {
        agentField = `<label>${label}</label><p class="muted">No captains for this provider yet — add them in Providers → Edit → Captains.</p><input id="agent" value="${esc(current || "")}" placeholder="Captain's name" />`;
      }
    }
    controls = `
      ${agentField}
  ...
  h(`
    ...
    <div class="card">
      ${badge(order.status)}
      ${order.agent_name ? `<p class="muted" style="margin-top:8px">🧢 Pickup: ${esc(order.agent_name)}${order.captain_phone ? ` · <a href="tel:${esc(order.captain_phone)}">${esc(order.captain_phone)}</a>` : ""}</p>` : ""}
      ${order.delivery_captain_name ? `<p class="muted" style="margin-top:4px">🛵 Delivery: ${esc(order.delivery_captain_name)}${order.delivery_captain_phone ? ` · <a href="tel:${esc(order.delivery_captain_phone)}">${esc(order.delivery_captain_phone)}</a>` : ""}</p>` : ""}
      <h2>Customer</h2>
