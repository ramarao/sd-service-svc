...
CREATE TABLE IF NOT EXISTS platform_settings (
  ...
  brand_name            TEXT,                   -- the city/town display name (set in the dashboard; overrides config)
  updated_at            INTEGER
);
...
CREATE TABLE IF NOT EXISTS users (
  ...
  must_change_password INTEGER NOT NULL DEFAULT 0,     -- 1 = force a new password on next login (first-run admin/admin)
  created_at  INTEGER NOT NULL
);
