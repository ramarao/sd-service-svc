...
async function renderDashboard(tab) {
  ...
  // First-run admin (admin/admin) must set a real password before anything else.
  if (who.must_change) return forcePasswordChange();
...
function adminLogin() {
  h(`
    <div class="topbar"><h1>${esc(CFG.brand?.name || "Admin")} · login</h1></div>
    <div class="card">
      <label>Email</label><input id="email" type="email" autocapitalize="off" autocomplete="username" placeholder="admin" />
      <label>Password</label><input id="password" type="password" autocomplete="current-password" />
      <button id="login" style="margin-top:12px">Log in</button>
      <p id="msg"></p>
      <p class="muted small" style="margin-top:12px">First time here? Log in with <b>admin</b> / <b>admin</b> — you'll set your own password next.</p>
    </div>`);
  document.getElementById("login").onclick = async () => {
    ...
    try {
      ...
      renderDashboard();
    } catch (e) { msg.className = "err"; msg.textContent = e.message === "invalid_credentials" ? "Wrong email or password." : e.message; }
  };
}

// First-run: the admin/admin account must set a real password (≥8 chars) before
// reaching the dashboard. No "current password" — they just logged in.
function forcePasswordChange() {
  h(`
    <div class="topbar"><h1>Set your password</h1></div>
    <div class="card">
      <p class="muted small" style="margin-top:0">Welcome! For security, choose a new password before you continue.</p>
      <label>New password <span class="muted small">— at least 8 characters</span></label>
      ...
      <button id="setpw" style="margin-top:12px">Save password</button>
      <p id="msg"></p>
    </div>`);
  document.getElementById("setpw").onclick = async () => {
    ...
    if (np.length < 8) { msg.className = "err"; msg.textContent = "Use at least 8 characters."; return; }
    if (np !== np2) { msg.className = "err"; msg.textContent = "Passwords don't match."; return; }
    try {
      await api("/api/account/set-password", { method: "POST", body: { next: np } });
      renderDashboard(); // session re-issued without must_change → dashboard opens
    } catch (e) { msg.className = "err"; msg.textContent = e.message === "too_short" ? "Use at least 8 characters." : e.message; }
  };
}
...
async function whatsappTab(container) {
  ...
  container.innerHTML = `
    <div class="card">
      <h2>City / brand name</h2>
      <p class="muted">The name shown to customers across the app and in WhatsApp messages.</p>
      <label>Display name</label>
      <input id="c_brand" value="${esc(settings.brand_name || CFG.brand?.name || "")}" placeholder="e.g. Tanuku" />
      <button id="c_save" style="margin-top:12px">Save name</button>
      <p id="c_msg"></p>
    </div>
  ...
  document.getElementById("c_save").onclick = async () => {
    ...
    try {
      await api("/api/console/settings", { method: "POST", body: { brand_name: document.getElementById("c_brand").value } });
      await loadConfig(); // refresh CFG.brand.name so the rest of the app updates
