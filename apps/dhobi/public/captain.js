...
async function screenOrders() {
  ...
  let data;
  try { data = await api(`/api/captain/orders?provider=${encodeURIComponent(state.providerId)}`); }
...
async function screenOrderDetail(orderId) {
  ...
  // Show the pickup contact only when it differs from the customer.
...
function renderPayment(orderId, d) {
  if (d.payment_status === "paid") {
    ...
    h(`<div class="topbar"><h1>Delivered ✓</h1><button class="ghost small" id="back">←</button></div>
       <div id="capPayMarker" hidden></div>
       <div class="card"><p>Order <strong>${esc(orderId)}</strong> delivered.</p><p class="muted">No UPI set up for this provider.</p></div>
