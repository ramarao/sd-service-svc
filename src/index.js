// sd-service-svc — multi-provider on-site service platform on Cloudflare Workers.
// One Worker serves: the WhatsApp webhook, the REST API, and the static SPA
// dashboards (customer / provider-admin / super-admin console).
import { Hono } from "hono";
import { setCookie, deleteCookie, getCookie } from "hono/cookie";
import QRCode from "qrcode";
import { handleEmail, processPaymentEmail } from "./email.js";
import { OrdersHub, notifyOrders } from "./orders-hub.js";
...
app.get("/api/captain/orders", requireRole("captain"), async (c) => {
  ...
  const jobs = await listCaptainJobs(c.env.DB, sess.phone, provider.id);
...
app.get("/api/captain/orders/:id", requireRole("captain"), async (c) => {
  ...
  const { results: catalog } = await c.env.DB.prepare(
    ...
    .all();
  // Only the pickup captain, and only before pickup, may edit the items.
  const editable = order.status === "ASSIGNED" && order.captain_phone === sess.phone;
  const action =
    order.status === "ASSIGNED" && order.captain_phone === sess.phone
      ? "pickup"
      : order.status === "OUT_FOR_DELIVERY" && order.delivery_captain_phone === sess.phone
      ? "deliver"
      : null;
  return c.json({ order, catalog, editable, action });
});
...
app.patch("/api/captain/orders/:id/items", requireRole("captain"), async (c) => {
  ...
  if (!order) return c.json({ error: "not_found" }, 404);
  if (order.captain_phone !== sess.phone || order.status !== "ASSIGNED") {
    return c.json({ error: "locked" }, 403); // not the pickup captain, or already picked up
  }
...
// Confirm pickup (ASSIGNED → PICKED_UP). Guarded: must be this captain's job.
app.post("/api/captain/orders/:id/pickup", requireRole("captain"), async (c) => {
  const sess = c.get("session");
  const order = await c.env.DB.prepare("SELECT * FROM orders WHERE id = ?").bind(c.req.param("id")).first();
  if (!order) return c.json({ error: "not_found" }, 404);
  if (order.captain_phone !== sess.phone || order.status !== "ASSIGNED") return c.json({ error: "forbidden" }, 403);
  try {
    const updated = await transitionOrder(c.env, c.env.DB, { orderId: order.id, toStatus: "PICKED_UP", actor: "captain" });
    await notifyOrders(c.env, order.provider_id);
...
// Confirm delivered (OUT_FOR_DELIVERY → DELIVERED). Guarded to this captain.
app.post("/api/captain/orders/:id/deliver", requireRole("captain"), async (c) => {
  const sess = c.get("session");
  const order = await c.env.DB.prepare("SELECT * FROM orders WHERE id = ?").bind(c.req.param("id")).first();
  if (!order) return c.json({ error: "not_found" }, 404);
  if (order.delivery_captain_phone !== sess.phone || order.status !== "OUT_FOR_DELIVERY") return c.json({ error: "forbidden" }, 403);
  try {
    const updated = await transitionOrder(c.env, c.env.DB, { orderId: order.id, toStatus: "DELIVERED", actor: "captain" });
    await notifyOrders(c.env, order.provider_id);
    return c.json({ ok: true, order: updated });
  } catch (e) {
    return c.json({ error: e.message }, 400);
...
app.get("/api/admin/orders/:id", requireRole("admin", "super_admin", "manager"), async (c) => {
  ...
  return c.json({ order, customer, allowedNext: allowedTransitions(order.status), captains });
});
...
app.get("/api/admin/meta", requireRole("admin", "super_admin", "manager"), (c) =>
  c.json({ statusFlow: STATUS_FLOW, terminal: ["REJECTED", "DELIVERED"] })
);
